...
var omise = require('omise')({'secretKey': 'skey_test_5a1j2ntf48hwtsp877s', 'omiseVersion': '2015-09-10'});
var mysqlPool = mysql.createPool({host: '172.104.189.169', user: 'root', password: 'my-secret-pw', database: 'tutordb'});
  ...
  omise.charges.create({
    ...
    if (resp.paid) {
      ...
      mysqlPool.getConnection(function(err, connection) {
        if (err)
          throw err;
        var query = "INSERT INTO `user_purchase`(`course_id`, `branch_id`, `user_id`, `purchase_ts`) VALUES (" +
        course_id + "," +
        branch_id + "," +
        user_id + ",'" + purchase_ts + "')"
